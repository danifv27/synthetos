...
func initializeVersionCmd(ctx floc.Context, ctrl floc.Control) error {
	...
	if c, err = KuberiumCmdCtx(ctx); err != nil {
		if e := KuberiumSetRCErrorTree(ctx, "initializeVersionCmd", err); e != nil {
	...
	if err = KuberiumSetCmdCtx(ctx, common.Cmdctx{
		Cmd:      c.Cmd,
		InitSeq:  c.InitSeq,
		Apps:     c.Apps,
		Adapters: c.Adapters,
...
func versionPrintJob(ctx floc.Context, ctrl floc.Control) error {
	...
	if c, err = KuberiumCmdCtx(ctx); err != nil {
		KuberiumSetRCErrorTree(ctx, "versionPrintJob", err)
		return err
	}
