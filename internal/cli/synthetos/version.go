...
func initializeVersionCmd(ctx floc.Context, ctrl floc.Control) error {
	...
	if c, err = SynthetosCmdCtx(ctx); err != nil {
		if e := SynthetosSetRCErrorTree(ctx, "initializeVersionCmd", err); e != nil {
	...
	if err = SynthetosSetCmdCtx(ctx, common.Cmdctx{
		Cmd:      c.Cmd,
		InitSeq:  c.InitSeq,
		Apps:     c.Apps,
		Adapters: c.Adapters,
...
func versionPrintJob(ctx floc.Context, ctrl floc.Control) error {
	...
	if c, err = SynthetosCmdCtx(ctx); err != nil {
		SynthetosSetRCErrorTree(ctx, "versionPrintJob", err)
		return err
	}
...
func (cmd *VersionCmd) Run(cli *CLI, c *common.Cmdctx, rcerror *error) error {
	...
	c.RunSeq = guard.OnTimeout(
		...
		func(ctx floc.Context, ctrl floc.Control, id interface{}) {
			...
			SynthetosSetRCErrorTree(ctx, "timeout", errors.New(msg))
			if rcerror, err := SynthetosRCErrorTree(ctx); err != nil {
				ctrl.Fail(fmt.Sprintf("Command '%s' internal error", c.Cmd), err)
			} else {
