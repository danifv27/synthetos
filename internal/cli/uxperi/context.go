...
func UxperiFlags(ctx floc.Context) (CLI, error) {
	...
	if cli, ok = ctx.Value(uxperiContextKeyCLI).(CLI); !ok {
		return CLI{}, errortree.Add(rcerror, "Flags", fmt.Errorf("type mismatch with key %s", uxperiContextKeyCLI))
	}
...
func UxperiRCErrorTree(ctx floc.Context) (*error, error) {
	...
	if obj == nil {
		...
		return nil, errortree.Add(rcerror, "RCErrorTree", fmt.Errorf("type mismatch with key %s", uxperiContextKeyRCError))
	}
...
func UxperiCmdCtx(ctx floc.Context) (*common.Cmdctx, error) {
	...
	if obj == nil {
		...
		return nil, errortree.Add(rcerror, "NewApplications", fmt.Errorf("type mismatch with key %s", uxperiContextKeyCmdCtx))
	}
