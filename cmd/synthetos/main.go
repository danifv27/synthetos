...
func initializeCmd(cli *synthetos.CLI, cmd string) (common.Cmdctx, error) {
...
func main() {
	...
	if result, data, err = floc.RunWith(flocCtx, ctrl, flow); err != nil {
		if rcerr, e := synthetos.SynthetosRCErrorTree(flocCtx); e != nil {
	...
	default:
		...
		if rcerror, err := synthetos.SynthetosRCErrorTree(flocCtx); err != nil {
			ctx.FatalIfErrorf(err)
		} else {
			ctx.FatalIfErrorf(*rcerror)
		}
